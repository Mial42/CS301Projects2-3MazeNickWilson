...
public class MazeBuilderKruskal extends MazeBuilder implements Runnable {
	 ...
	 * Create a list of all possible edges in the Maze. By default, edges are blocked (there is a wallboard between the two nodes neighboring the edge)
	 * Then, loop until the list of edges is empty. Within that loop:
	 * 1) Remove the next edge specified by the seed from the list of edges.
	 * 2) Calculate the endpoint of the edge (for instance, if the edge has x coord 2, y coord 2, and points south, it's connected to x coord 2, y coord 3).
	 * 3) Create references to the trees corresponding to the start and end points of the edge.
	 * 4)If the two nodes are not in the same Tree, merge their Trees and remove the appropriate wall from the maze.
	 */
	@Override
	 ...
	 * Create all possible edges by going to the right for edges between horizontal nodes. Skip the last column to avoid the borders.
	 * And down for edges between vertical nodes. Skip the last row to avoid the borders
